...
const FacultyRooms: React.FC = () => {
  ...
  useEffect(() => {
    // Only show rooms from the faculty's department
    ...
    <div className="page-transition">
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <p className="text-gray-600">
          Find and book rooms in the {user?.department} department
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRooms.map(room => (
            <RoomCard key={room.id} room={room} linkPrefix="/faculty" />
          ))}
        </div>
      ) : (
